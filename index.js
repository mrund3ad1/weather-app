...
//     let fullURL = "https://api.openweathermap.org/data/2.5/weather?q=a&units=imperial&appid=" + process.env.API_KEY;
...
function openWeather(city, unit, res) {
    const fullURL = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&units=" + unit + "&appid=" + process.env.API_KEY;
    https.get(fullURL, function (response) {
        response.on("data", function (data) {
            const wData = JSON.parse(data);
